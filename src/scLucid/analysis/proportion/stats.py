...
def _run_deseq2(
    ...
    metadata = pd.DataFrame({
        condition_col: sample_to_cond
    })

    # Filter low counts
    counts_filtered = Preprocessing.filter_genes(
        counts, min_cells=1, min_counts=10
    )
...
def _run_ttest(
    ...
    if len(conditions) != 2:
        log.warning(
            "t-test requires exactly 2 conditions. "
...
def _run_wilcoxon(
    ...
    if len(conditions) != 2:
        log.warning(
            "Wilcoxon test requires exactly 2 conditions. "
            f"Got {len(conditions)}."
        )
        return pd.DataFrame()
...
def _run_anova(
    ...
    for celltype in count_df.columns:
        groups = [
            count_df.loc[sample_to_cond == cond, celltype]
...
def _run_paired_test(
    ...
    if len(conditions) != 2:
        log.warning(
            "Paired test requires exactly 2 conditions. "
            f"Got {len(conditions)}."
        )
        return pd.DataFrame()
    ...
    for celltype in count_df.columns:
        ...
        for pair_id in sample_to_pair.unique():
            ...
            if len(pair_samples) == 2:
                cond1_val = count_df.loc[
                    pair_samples[sample_to_cond.loc[pair_samples] == conditions[0]],
                    celltype
                ].values

                cond2_val = count_df.loc[
                    pair_samples[sample_to_cond.loc[pair_samples] == conditions[1]],
                    celltype
                ].values
        ...
        if len(pairs) < 3:
            log.warning(
                f"Insufficient pairs for {celltype}: {len(pairs)}. "
                "Skipping."
            )
            continue
...
def run_statistical_test(
    ...
    if test_method == 'deseq2':
        ...
        res_df = _run_paired_test(
            count_df, sample_to_cond, sample_to_pair, 't-test'
        )
    elif test_method == 'paired-wilcoxon':
        if sample_to_pair is None:
            raise ValueError("sample_to_pair required for paired tests")
        res_df = _run_paired_test(
            count_df, sample_to_cond, sample_to_pair, 'wilcoxon'
        )
    else:
...
def _add_effect_sizes(
    ...
    if len(conditions) != 2:
        log.warning(
            "Effect size calculation requires exactly 2 conditions. "
            f"Got {len(conditions)}."
        )
        return res_df
