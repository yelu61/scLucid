...
def run_annotation(
    ...
    """
    Run a comprehensive, multi-evidence cell type annotation workflow.

    This function orchestrates a full annotation pipeline:
    1. (Optional) Runs CellTypist for an initial automated annotation.
    2. (Optional) Calculates marker scores for all known cell types.
    3. Annotates clusters using a final evidence-based method.
    """
    log.info("\n" + "="*50)
    log.info("=== Starting Cell Type Annotation Workflow ===")
    log.info("="*50)

    # 1. Load the appropriate marker manager
    log.info(f"Loading markers for species='{config.marker_species}', tissue='{config.marker_tissue}'")
    marker_mgr = get_marker_manager(species=config.marker_species, tissue=config.marker_tissue)
    marker_mgr.intersect_with(adata.raw if adata.raw is not None else adata)
    
    # 2. (Optional) Run CellTypist
    if config.run_celltypist:
        log.info("\n---- Step 1: Running CellTypist ----")
        adata = run_celltypist(adata, model=config.celltypist_model)

    # 3. (Optional) Score cell types based on marker manager
    if config.run_scoring:
        log.info("\n---- Step 2: Scoring Cell Types with Marker Sets ----")
        adata = score_cell_types(adata, marker_config=marker_mgr, use_raw=True)

    # 4. Run final annotation method
    log.info(f"\n---- Step 3: Final Annotation using '{config.final_method}' method ----")
    adata = annotate_clusters(
        ...
        use_raw=True # Recommended for enrichment
    )
    
    # Store the config used for this run
    adata.uns.setdefault('scrnatk', {}).setdefault('analysis', {}).setdefault('annotation', {})
    adata.uns['scrnatk']['analysis']['annotation']['workflow_config'] = config.__dict__
    
    log.info("\n" + "="*50)
    log.info("=== Annotation Workflow Complete! ===")
    log.info(f"Final annotations are in .obs['{config.key_added}']")
    log.info("="*50)
