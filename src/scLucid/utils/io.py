"""User-facing ``.h5ad`` reader and 10x Genomics loader for scLucid.
...
from .helpers import assess_matrix_semantics, subset_adata
from .sanitize import sanitize_for_hdf5
from .storage import write_h5ad_safe
...
def _looks_like_counts(matrix) -> bool:
    ...
    try:
        ...
        return bool(result["is_count_like"])
    except Exception:
        return False
...
def write_seurat_conversion_script(
    h5ad_path: Union[str, Path],
    *,
    output_dir: Optional[Union[str, Path]] = None,
    output_stem: Optional[str] = None,
    formats: Iterable[Literal["h5seurat", "rds", "rdata"]] = ("h5seurat", "rds"),
    overwrite: bool = True,
) -> Path:
    """Write an R script for converting an exported ``.h5ad`` to Seurat formats.

    The script is intentionally not executed by Python. It records the common
    SeuratDisk conversion pattern used in project notebooks while making the
    R dependency boundary explicit.
    """
    h5ad = Path(h5ad_path).expanduser().resolve()
    if h5ad.suffix.lower() != ".h5ad":
        raise ValueError("h5ad_path must point to a .h5ad file")

    out_dir = Path(output_dir).expanduser().resolve() if output_dir else h5ad.parent
    out_dir.mkdir(parents=True, exist_ok=True)
    stem = output_stem or h5ad.stem
    script_path = out_dir / f"{stem}_h5ad_to_seurat.R"

    valid_formats = {"h5seurat", "rds", "rdata"}
    requested = [str(fmt).lower() for fmt in formats]
    invalid = sorted(set(requested) - valid_formats)
    if invalid:
        raise ValueError(f"Unsupported Seurat export formats: {invalid}")
    if not requested:
        raise ValueError("At least one Seurat export format must be requested")

    overwrite_r = "TRUE" if overwrite else "FALSE"
    h5ad_r = str(h5ad).replace("\\", "/")
    script = [
        "# Auto-generated by scLucid. Review paths and installed R packages before running.",
        "suppressPackageStartupMessages({",
        "  library(Seurat)",
        "  library(SeuratDisk)",
        "})",
        "",
        f"h5ad_path <- normalizePath({h5ad_r!r}, mustWork = TRUE)",
        f"out_dir <- normalizePath({str(out_dir).replace(chr(92), '/')!r}, mustWork = TRUE)",
        f"output_stem <- {stem!r}",
        "h5seurat_path <- file.path(out_dir, paste0(output_stem, '.h5seurat'))",
        "rds_path <- file.path(out_dir, paste0(output_stem, '.rds'))",
        "rdata_path <- file.path(out_dir, paste0(output_stem, '.RData'))",
        "",
    ]
    if "h5seurat" in requested or "rds" in requested or "rdata" in requested:
        script.extend(
            [
                "# SeuratDisk writes the .h5seurat next to the source h5ad.",
                f"Convert(h5ad_path, dest = 'h5seurat', overwrite = {overwrite_r})",
                "generated_h5seurat <- sub('\\\\.h5ad$', '.h5seurat', h5ad_path)",
                "if (!identical(normalizePath(generated_h5seurat), normalizePath(h5seurat_path))) {",
                f"  if (file.exists(h5seurat_path) && !{overwrite_r}) {{",
                "    stop('Target h5Seurat exists: ', h5seurat_path)",
                "  }",
                "  file.copy(generated_h5seurat, h5seurat_path, overwrite = TRUE)",
                "}",
    ...
    if "rds" in requested:
        script.append("saveRDS(seurat_obj, rds_path)")
    if "rdata" in requested:
        script.append("save(seurat_obj, file = rdata_path)")
    script.append("")
...
def export_adata_subset(
    ...
    *,
    ...
    write_r_script: bool = True,
    seurat_formats: Iterable[Literal["h5seurat", "rds", "rdata"]] = ("h5seurat", "rds"),
) -> Dict[str, Any]:
    """Export a reusable AnnData subset plus notebook handoff sidecars.

    This is the stable API version of repeated project-notebook patterns:
    subset cells by ``obs`` annotations, write a clean ``.h5ad``, export
    selected metadata, and optionally create an R conversion script for Seurat.
    It does not execute R conversion.
    """
    if not prefix:
    ...
    subset = subset_adata(
        adata,
        filters=filters or {},
        keep_raw_genes=keep_raw_genes,
    )
    if obs_columns is not None:
        missing = [col for col in obs_columns if col not in subset.obs.columns]
        if missing:
            raise KeyError(f"obs_columns not found in adata.obs: {missing}")

    out_dir = Path(output_dir).expanduser().resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    h5ad_path = out_dir / f"{prefix}.h5ad"
    written_h5ad = write_h5ad_safe(
        subset,
        h5ad_path,
    ...
    artifacts: Dict[str, Path] = {"h5ad": written_h5ad}
    if write_metadata:
        if obs_columns is None:
            metadata = subset.obs.copy()
        else:
            metadata = subset.obs[obs_columns].copy()
        metadata_path = out_dir / f"{prefix}_obs_metadata.csv"
        metadata.to_csv(metadata_path)
        artifacts["metadata"] = metadata_path

    r_script_path: Optional[Path] = None
    if write_r_script:
        r_script_path = write_seurat_conversion_script(
            written_h5ad,
            output_dir=out_dir,
            output_stem=prefix,
            formats=seurat_formats,
        )
        artifacts["seurat_script"] = r_script_path

    manifest: Dict[str, Any] = {
        "schema_version": "sclucid_subset_export_v1",
        "created_at": datetime.now().isoformat(),
        "prefix": prefix,
        "filters": sanitize_for_hdf5(filters or {}),
        "n_obs": int(subset.n_obs),
        "n_vars": int(subset.n_vars),
        "raw_n_vars": int(subset.raw.n_vars) if subset.raw is not None else None,
        "obs_columns_exported": list(obs_columns)
        if obs_columns is not None
        else list(subset.obs.columns),
        "artifacts": {key: str(path) for key, path in artifacts.items()},
        "r_conversion_executed": False,
    }
    if write_manifest:
        manifest_path = out_dir / f"{prefix}_export_manifest.json"
        manifest["artifacts"]["manifest"] = str(manifest_path)
        manifest_path.write_text(
            json.dumps(sanitize_for_hdf5(manifest), indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
    ...
    *,
    ...
    if path_obj.is_dir():
        ...
        try:
            ...
            log.warning(
                "scanpy.read_10x_mtx failed for %s: %s. Falling back to manual mtx parser.",
                path_obj,
                exc,
            )
...
def _read_10x_multi(
    *,
    ...
    if len(valid_samples) < len(samples):
        log.warning("Found valid paths for %d/%d samples", len(valid_samples), len(samples))
    ...
    log.info("Combined dataset: %d cells, %d genes", combined.n_obs, combined.n_vars)

    if output_file:
        out_path = Path(output_file).expanduser()
        if out_path.exists() and backup_existing:
            backup_path = out_path.with_name(f"{out_path.name}.bak.{int(time.time())}")
...
def _read_10x_manually(sample_path: str) -> AnnData:
    ...
    if X.shape[0] != len(barcodes):
        raise ValueError(f"Mismatch: {X.shape[0]} cells in matrix, but {len(barcodes)} barcodes")

    if X.shape[1] != len(gene_names):
        raise ValueError(
            f"Mismatch: {X.shape[1]} genes in matrix, but {len(gene_names)} gene names"
        )
