...
__all__ = ["normalize_data", "regress_out", "plot_normalization_comparison"]


# --- Helper Function to Plot data distribution ---
...
def normalize_data(
    ...
    log.info(f"Beginning data normalization using '{method}' method.")
    log.info(f"Parameters: target_sum={target_sum}, log_transform={log_transform}")

    # Store original data state for plotting and reference
    ...
    # Check for zero or negative values that might cause problems
    min_value = np.min(input_data_source)
    if min_value < 0:
        raise ValueError(
            f"Input data contains negative values (min={min_value}). Normalization requires non-negative values."
        )
    ...
    if np.min(total_counts) == 0:
        log.warning(
            "Some cells have zero total counts. These cells may cause issues with normalization."
        )
    ...
    if plot:
        try:
            ...
            gene_fig = plot_normalization_comparison(
                adata,
