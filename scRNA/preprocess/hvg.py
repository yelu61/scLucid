...
def _get_sample_specific_genes(
    ...
    if adata.obs[sample_key].nunique() <= 1:
        print(
            "Info: Only one sample group found. Skipping sample-specific gene identification."
        )
        return []
...
def annotate_hvg(
    ...
    with use_layer_as_X(adata, layer):
        if method == "scanpy":
            ...
            print(
                f"Found {combined_hvgs.sum()} genes considered HVG in at least {min_n_samples} samples."
            )

            # 3. Identify genes to exclude
            # Highly expressed genes
            gene_expr = np.array(adata.X.sum(axis=0)).flatten()
            top_expr_genes = adata.var_names[
                np.argsort(-gene_expr)[:n_highly_expressed_genes]
            ]

            # Sample-specific genes
            specific_genes = _get_sample_specific_genes(
                adata, sample_key, n_specific_genes
            )

            exclude_genes = set(top_expr_genes) | set(specific_genes)
            print(
                f"Identified {len(exclude_genes)} highly expressed or sample-specific genes to exclude."
            )
    ...
    print(
        f"Final number of highly variable genes ({method} method): {adata.var[output_key].sum()}"
    )
    return adata
...
def select_hvg(
    ...
    if n_top_genes is not None:
        if "dispersions_norm" not in adata.var:
            raise KeyError(
                "`dispersions_norm` not found in `adata.var`. Needed to select top N genes."
            )
