...
def _load_cell_cycle_genes():
    ...
    try:
        ...
        print(
            "Error: cell_cycle_genes.json not found. Please ensure the package is installed correctly."
        )
        return {}
...
def score_cell_cycle(
    ...
    if s_genes is None or g2m_genes is None:
        if species not in SPECIES_GENES:
            raise ValueError(
                f"Unknown species: '{species}'. Valid options are: {', '.join(SPECIES_GENES.keys())}"
            )
        ...
        if species == "human" and not any(
            ...
            for sp in ["mouse", "rat"]:
                if any(g in adata.var_names for g in SPECIES_GENES[sp]["s_genes"][:10]):
                    print(
                        f"Warning: Default species is 'human', but '{sp}' genes were detected. Switching gene list to '{sp}'."
                    )
                    current_species = sp
                    break
    ...
    if len(s_genes_found) < 5 or len(g2m_genes_found) < 5:
        print(
            f"Warning: Few cell cycle genes found in data (S: {len(s_genes_found)}, G2M: {len(g2m_genes_found)})."
        )
        print(
    ...
    if plot:
        ...
        fig.suptitle(
            f"Cell Cycle Analysis ({current_species.capitalize()})", fontsize=16
        )
        ...
        if save_dir:
            ...
            plt.savefig(
                os.path.join(save_dir, f"cell_cycle_scores_{current_species}.png"),
                dpi=300,
                bbox_inches="tight",
            )
        plt.show()
