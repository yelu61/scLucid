#!/usr/bin/env python3
"""Build a unified QC evidence package from validation outputs."""
...
def _source_rows_from_figure_table(
    ...
    *,
    ...
    for _, row in table.iterrows():
        ...
        source_panel = str(row.get("module_panel", row.get("figure_panel")))
        module_panel = panel_map.get(source_panel, source_panel) if panel_map else source_panel
        rows.append(
            {
        ...
        rows.append(
            {
                ...
                "qc_panels": ";".join(spec.figure2_panels),
    ...
    rows.append(
        {
            ...
            "next_action": "Convert PDAC/NSCLC/CRC results into QC tumor reviewer reviewer panels.",
        }
    )
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "## Dataset Coverage",
            "",
            "| Dataset | Available | Role | QC evidence panels | Notes |",
            "|---|---:|---|---|---|",
        ]
    )
...
def build_package(input_root: Path, output_dir: Path) -> dict[str, Path]:
    ...
    paths = {
        "qc_source_data": output_dir / "qc_source_data.tsv",
    ...
    pd.DataFrame(source_rows).to_csv(paths["qc_source_data"], sep="\t", index=False)
