...
**Core Dependencies**: - scanpy \>= 1.10 - anndata \>= 0.10 - pandas \>=
1.5 - numpy \>= 1.23 - scipy \>= 1.10 - pydantic \>= 2.0
...
QC/preprocess path. Prefer Python-native defaults and use Scanpy's
optional `scanpy.external` integrations only when the environment is
already prepared.
...
- `quickstart` - Get started with a basic workflow
...
- `best_practices` - Recommended practices for scLucid usage
