...
- `scLucid.qc.filtering.generate_qc_report` remains as a compatibility
  wrapper only; filtering code should stay focused on threshold
  suggestion, marking, filtering, and filtering audit. New code should
  use the reporting entrypoint; the filtering wrapper emits a
  `FutureWarning`.
