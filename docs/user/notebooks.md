...
Project notebooks are intentionally kept outside the documentation
source tree. In the current repository layout they live under
...
- **notebooks** show complete analysis stories, often with real data,
  richer plots, intermediate reasoning, and reviewer-facing outputs

For a package that is being prepared for publication, this separation
helps keep the stable documentation concise while still preserving full
analysis narratives.
...
Do not use notebooks as the only place where the recommended package
path is defined. That guidance should remain in `quickstart` and
`best_practices`.
...
- `Step1B-Preprocessing_Audit.ipynb` - benchmark-grade preprocessing
  audit notebook with QC handoff context, layer transition evidence, and
...
- `Step2-Annotation_and_Malignancy.ipynb` - evidence-first analysis
  ...
  CNV-aware interpretation. It writes
...
that explains their scope and expected inputs.
